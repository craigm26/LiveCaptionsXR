...
class EnhancedSpeechProcessor {
  ...
  Future<SpeechResult?> debugTranscribeWavFile(String wavFilePath) async {
    if (!_isInitialized) {
      _logger.w('debugTranscribeWavFile called before initialization',
          category: LogCategory.speech);
      return null;
    }

    try {
      _logger.i('🧪 Running direct debug WAV transcription: $wavFilePath',
          category: LogCategory.speech);
  ...
  Future<void> _maybeDumpDebugWav(Uint8List pcm16LeBytes,
    ...
    try {
      ...
      _logger.i('🧪 [AUDIO DUMP] Wrote live mic WAV sample to ${file.path}',
          category: LogCategory.speech);
    } catch (e, stackTrace) {
      _logger.w('⚠️ [AUDIO DUMP] Failed to write debug WAV: $e',
          category: LogCategory.speech);
      _logger.d('ℹ️ [AUDIO DUMP] Write failure stack',
  ...
  EnhancedSpeechProcessor({
    ...
    _logger.i('🏗️ [DEBUG] EnhancedSpeechProcessor constructor called',
        category: LogCategory.speech);
    _logger.i(
        '🎤 [DEBUG] AppleSpeechService instance: ${_appleSpeechService.runtimeType}',
        category: LogCategory.speech);
    _logger.i('🔧 [DEBUG] Active engine set to: $_activeEngine',
        category: LogCategory.speech);
    _logger.i('🍎 [DEBUG] Platform.isIOS: ${Platform.isIOS}',
        category: LogCategory.speech);
    _logger.i('🚀 [DEBUG] NexaAsrService available: ${_nexaAsrService != null}',
        category: LogCategory.speech);
    _logger.i('🚀 [DEBUG] NexaLlmService available: ${_nexaLlmService != null}',
        category: LogCategory.speech);
  ...
  static SpeechEngine _getDefaultEngine() {
    ...
    logger.i('🔧 [DEBUG] _getDefaultEngine called',
        category: LogCategory.speech);
    if (Platform.isIOS) {
      logger.i('🍎 [DEBUG] iOS detected - returning apple_speech engine',
          category: LogCategory.speech);
      ...
      logger.i(
          '🤖 [DEBUG] Android detected - returning nexa_asr engine (NPU accelerated)',
          category: LogCategory.speech);
      return SpeechEngine.nexa_asr;
    }
    logger.i('🖥️ [DEBUG] Other platform - returning flutter_sound engine',
        category: LogCategory.speech);
  ...
  Future<bool> initialize({
    ...
    try {
      ...
      _logger.i('📸 Initializing FrameCaptureService...',
      ...
      if (frameInitialized) {
        ...
        if (!Platform.isIOS) {
          ...
          _logger.d('🤖 Android detected - frames will be captured on-demand',
              category: LogCategory.camera);
        } else {
          _logger.d('🍎 iOS detected - ARKit frames will be captured on-demand',
      ...
      final isNexaDevice =
          _activeEngine == SpeechEngine.nexa_asr && _nexaLlmService != null;
      if (enableGemmaEnhancement && !isNexaDevice) {
        _logger.i('🤖 Checking Gemma3n service for enhancement...',
            category: LogCategory.gemma);

        if (gemma3nService.isReady) {
          _logger.i('✅ Gemma3n service already ready (pre-initialized)',
              category: LogCategory.gemma);
        } else {
          _logger.i(
          ...
          try {
            ...
            if (gemma3nService.isReady) {
              _logger.i('✅ Gemma3n service initialized successfully',
                  category: LogCategory.gemma);
            } else {
              _logger.w(
            ...
            _logger.e('⏱️ Gemma3n service initialization timed out',
                category: LogCategory.gemma, error: e);
            _logger.w('⚠️ Gemma enhancement will be disabled due to timeout',
                category: LogCategory.gemma);
          } catch (e, stackTrace) {
            _logger.e('❌ Failed to initialize Gemma3n service',
                category: LogCategory.gemma, error: e, stackTrace: stackTrace);
            _logger.w('⚠️ Gemma enhancement will be disabled due to error',
                category: LogCategory.gemma);
          }
        }
      } else if (isNexaDevice) {
        _logger.i(
            '🤖 Nexa device detected — skipping Gemma init, using Nexa LLM for enhancement',
            category: LogCategory.gemma);
      }
      ...
      switch (_activeEngine) {
        ...
        case SpeechEngine.apple_speech:
          _logger.i('🍎 [DEBUG] About to initialize Apple Speech',
              category: LogCategory.speech);
          await _initializeAppleSpeech();
          _logger.i('🍎 [DEBUG] Apple Speech initialization completed',
              category: LogCategory.speech);
          break;
        case SpeechEngine.nexa_asr:
          if (Platform.isAndroid) {
            _logger.i('🚀 [DEBUG] About to initialize Nexa ASR',
                category: LogCategory.speech);
            final nexaSuccess = await _initializeNexaAsr();
            if (!nexaSuccess) {
              _logger.w(
                  '⚠️ Nexa ASR initialization failed, falling back to Whisper GGML',
                  category: LogCategory.speech);
              _activeEngine = SpeechEngine.whisper_ggml;
              await _initializeWhisperGgml();
            }
            _logger.i('🚀 [DEBUG] Nexa ASR initialization completed',
                category: LogCategory.speech);
          } else {
            _logger.w(
                '⚠️ Nexa ASR only available on Android, falling back to platform default',
                category: LogCategory.speech);
            _activeEngine = Platform.isIOS
                ? SpeechEngine.apple_speech
                : SpeechEngine.flutter_sound;
            if (Platform.isIOS) {
              await _initializeAppleSpeech();
            } else {
              await _initializeFlutterSound();
  ...
  Future<void> _initializeWhisperGgml() async {
    try {
      ...
      _logger.e('❌ Failed to initialize Whisper GGML',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
      ...
      _logger.w(
          '⚠️ Whisper fallback unavailable — processor will run without ASR fallback',
          category: LogCategory.speech);
    }
  }
  ...
  Future<void> _initializeAppleSpeech() async {
    try {
      _logger.i('🍎 [DEBUG] _initializeAppleSpeech called',
          category: LogCategory.speech);
      _logger.i('🍎 [DEBUG] AppleSpeechService instance available',
          category: LogCategory.speech);
      ...
      await _appleSpeechService
        ...
        throw TimeoutException(
            'Apple Speech initialization timed out', Duration(seconds: 30));
      });

      _logger.i('✅ Apple Speech engine initialized',
          category: LogCategory.speech);
      _logger.i(
          '🍎 [DEBUG] Apple Speech isInitialized: ${_appleSpeechService.isInitialized}',
          category: LogCategory.speech);
    } catch (e, stackTrace) {
      _logger.e('❌ Failed to initialize Apple Speech',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<bool> _initializeNexaAsr() async {
    if (_nexaAsrService == null) {
      _logger.w('⚠️ NexaAsrService not available',
          category: LogCategory.speech);
      return false;
    }

    try {
      _logger.i('🚀 [DEBUG] _initializeNexaAsr called',
          category: LogCategory.speech);

      // Check NPU availability
      final npuAvailable = await _nexaAsrService!.isNpuAvailable();
      _logger.i('🚀 [DEBUG] NPU available: $npuAvailable',
          category: LogCategory.speech);

      // Initialize with NPU preference
      final success = await _nexaAsrService!
          .initialize(
        config: _config,
        preferNpu: true,
      )
        ...
        _logger.e(
            '⏰ [DEBUG] NexaAsrService.initialize() timed out after 60 seconds',
            category: LogCategory.speech);
        return false;
      });
      ...
      if (_nexaLlmService != null && !_nexaLlmService!.isInitialized) {
        _logger.i('🚀 Starting Nexa LLM initialization (parallel with ASR)...',
            category: LogCategory.gemma);
        ...
        _nexaLlmService!
            ...
            .then((_) {
          _logger.i('✅ Nexa LLM initialized for enhancement',
              category: LogCategory.gemma);
        }).catchError((e) {
          _logger.w('⚠️ Nexa LLM initialization failed, enhancement disabled',
              category: LogCategory.gemma, error: e);
        });
      }
      ...
      if (success) {
        _logger.i(
            '✅ Nexa ASR engine initialized (mode: ${_nexaAsrService!.inferenceMode.name.toUpperCase()})',
            category: LogCategory.speech);
        return true;
      } else {
        _logger.w('⚠️ Nexa ASR initialization returned false',
            category: LogCategory.speech);
        return false;
      }
    } catch (e, stackTrace) {
  ...
  Future<bool> startProcessing({SpeechConfig? config}) async {
    ...
    try {
      ...
      // Removed native speech fallback logic for Whisper to enforce whisper STT on emulators
      ...
      switch (_activeEngine) {
        ...
        case SpeechEngine.whisper_ggml:
          ...
          break;
        case SpeechEngine.apple_speech:
          _logger.i('🍎 [DEBUG] About to start Apple Speech processing',
              category: LogCategory.speech);
          await _startAppleSpeechProcessing();
          _logger.i('🍎 [DEBUG] Apple Speech processing started',
              category: LogCategory.speech);
          break;
        case SpeechEngine.nexa_asr:
          _logger.i('🚀 [DEBUG] About to start Nexa ASR processing',
              category: LogCategory.speech);
          await _startNexaAsrProcessing();
          _logger.i('🚀 [DEBUG] Nexa ASR processing started',
              category: LogCategory.speech);
          break;
      }
    ...
    _recorderSubscription =
      ...
      if (buffer is FoodData && buffer.data != null) {
        try {
          ...
          switch (_activeEngine) {
            ...
            case SpeechEngine.whisper_ggml:
              ...
              if (_whisperService.isInitialized) {
                final result =
                    await _whisperService.processAudioBuffer(buffer.data!);
          ...
          _logger.e('Error transcribing audio',
              category: LogCategory.speech, error: e);
        }
      }
    });
    ...
    try {
      ...
      _audioSubscription =
        ...
        if (_whisperChunkLogCounter % 10 == 0) {
          _logger.d(
              '🎵 Received audio chunk (${audioData.length} samples, sample #$_whisperChunkLogCounter)',
              category: LogCategory.speech);
        }
        ...
        // Keep a sliding window to prevent stale audio buildup while Whisper is busy processing
        final maxBufferSize = requiredSamples * 2;
        if (_whisperSampleBuffer.length > maxBufferSize) {
          _whisperSampleBuffer.removeRange(
              0, _whisperSampleBuffer.length - maxBufferSize);
        }
        ...
        if (_whisperTranscriptionInFlight) {
          // Whisper is busy processing the previous chunk. Let the buffer accumulate
          // (capped by the sliding window above) so the next inference is up-to-date.
          return;
        }
        ...
        try {
          ...
          if (rms < rmsThreshold) {
            ...
            _logger.i(
                '🎙️ Forcing Whisper inference despite low RMS to keep captions responsive',
                category: LogCategory.speech);
          }
          ...
          unawaited(
              _maybeDumpDebugWav(audioBuffer, tag: 'emulator-live-window'));
          _logger.d(
              '🔄 Converting audio to bytes (${audioBuffer.length} bytes)',
              category: LogCategory.speech);

          // Process with Whisper service
          _logger.d(
              '🎤 Sending audio to Whisper for transcription... (inFlightMs=0)',
              category: LogCategory.speech);
          final result = await _whisperService.processAudioBuffer(audioBuffer);

          if (_audioSubscription == null) {
            _logger.d(
                '⏹️ Ignoring Whisper result because processing has already stopped',
                category: LogCategory.speech);
            return;
          }
          ...
          final inferenceMs = _whisperInferenceStartedAt == null
              ? -1
              : DateTime.now()
                  .difference(_whisperInferenceStartedAt!)
                  .inMilliseconds;
          _logger.i(
              '📝 Whisper transcription result: "$normalizedText" (confidence: ${result.confidence}, inFlightMs: $inferenceMs, original: "${result.text}")',
              category: LogCategory.speech);

          if (normalizedText.isEmpty ||
              normalizedText.toLowerCase() == 'listening...') {
            _emptyWhisperWindows++;
            if (_emptyWhisperWindows % 4 == 0) {
              _logger.i(
                  '🫧 Whisper returned empty transcripts for $_emptyWhisperWindows windows; emitting activity fallback',
                  category: LogCategory.speech);
              _processSpeechResult(SpeechResult(
            ...
            _logger.d('⏭️ Ignoring empty Whisper transcript window',
                category: LogCategory.speech);
            return;
          }
          ...
          final inferenceMs = _whisperInferenceStartedAt == null
              ...
              : DateTime.now()
                  .difference(_whisperInferenceStartedAt!)
                  .inMilliseconds;
          _logger.e('❌ Error processing audio chunk (inFlightMs: $inferenceMs)',
              category: LogCategory.speech, error: e, stackTrace: stackTrace);
        } finally {
        ...
        _logger.e('❌ Error in audio stream',
            category: LogCategory.speech, error: error, stackTrace: stackTrace);
      });
      ...
      _logger.e('❌ Failed to start Whisper GGML processing',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> _startAppleSpeechProcessing() async {
    try {
      _logger.i('🎤🍎 [APPLE STT] Starting Apple Speech processing...',
          category: LogCategory.speech);
      _logger.i(
      ...
      _appleSpeechSubscription =
        ...
        _logger.i(
            ...
            category: LogCategory.speech);
        _logger.i('🔄 [APPLE STT] Forwarding to _processSpeechResult...',
            category: LogCategory.speech);
        _processSpeechResult(result);
      }, onError: (error, stackTrace) {
        _logger.e('❌ [APPLE STT] Error in Apple Speech stream',
            category: LogCategory.speech, error: error, stackTrace: stackTrace);
      });
      ...
      unawaited(
        _appleSpeechService
            .startProcessing(useOfflineMode: useOfflineMode)
            .then((success) {
          _logger.i(
              '🍎 [DEBUG] _appleSpeechService.startProcessing() returned: $success',
              category: LogCategory.speech);
          if (!success) {
            _logger.w('⚠️ [APPLE STT] Apple Speech start returned false',
                category: LogCategory.speech);
          }
        }).catchError((error, stackTrace) {
          _logger.e('❌ [APPLE STT] Async start failed',
              category: LogCategory.speech,
              error: error,
              stackTrace: stackTrace);
        }),
      ...
      _logger.i(
          '✅ [APPLE STT] Apple Speech processing launch requested (non-blocking)',
          category: LogCategory.speech);
    } catch (e, stackTrace) {
      _logger.e('❌ [APPLE STT] Failed to start Apple Speech processing',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> _startNexaAsrProcessing() async {
    if (_nexaAsrService == null) {
      _logger.e('❌ [NEXA STT] NexaAsrService not available',
          category: LogCategory.speech);
      throw StateError('NexaAsrService not available');
    }

    try {
      _logger.i('🎤🚀 [NEXA STT] Starting Nexa ASR processing...',
          category: LogCategory.speech);
      _logger.i(
          '🚀 [DEBUG] Nexa ASR service initialized check: ${_nexaAsrService!.isInitialized}',
          category: LogCategory.speech);
      _logger.i(
          '🚀 [DEBUG] Nexa ASR inference mode: ${_nexaAsrService!.inferenceMode.name.toUpperCase()}',
          category: LogCategory.speech);

      // Subscribe to Nexa ASR results
      _nexaAsrService!.speechResults.listen((result) {
        _logger.i(
            '🎤📥 [NEXA STT] Received result from NexaAsrService: "${result.text}" (confidence: ${result.confidence}, final: ${result.isFinal})',
            category: LogCategory.speech);
        _logger.i('🔄 [NEXA STT] Forwarding to _processSpeechResult...',
            category: LogCategory.speech);
        _processSpeechResultWithNexa(result);
      }, onError: (error, stackTrace) {
        _logger.e('❌ [NEXA STT] Error in Nexa ASR stream',
            category: LogCategory.speech, error: error, stackTrace: stackTrace);
      });
      ...
      _audioSubscription =
        ...
        if (_nexaChunkLogCounter % 10 == 0) {
          _logger.d(
              '🎵 [NEXA STT] Received audio chunk (${audioData.length} samples, sample #$_nexaChunkLogCounter)',
              category: LogCategory.speech);
        }
        ...
        try {
          ...
          _logger.d(
              '🔄 [NEXA STT] Converting ${audioData.length} samples to ${audioBytes.length} PCM bytes',
              category: LogCategory.speech);

          // Process with Nexa ASR service
          _logger.d(
              '🎤 [NEXA STT] Sending audio to Nexa ASR for transcription...',
              category: LogCategory.speech);
          await _nexaAsrService!.processAudioBuffer(audioBytes);
        } catch (e, stackTrace) {
          _logger.e('❌ [NEXA STT] Error processing audio chunk',
              category: LogCategory.speech, error: e, stackTrace: stackTrace);
        }
      }, onError: (error, stackTrace) {
        _logger.e('❌ [NEXA STT] Error in audio stream',
            category: LogCategory.speech, error: error, stackTrace: stackTrace);
      });
      ...
      _logger.i(
          '🚀 [DEBUG] _nexaAsrService.startProcessing() returned: $success',
          category: LogCategory.speech);

      _logger.i(
          '✅ [NEXA STT] Nexa ASR processing started successfully (mode: ${_nexaAsrService!.inferenceMode.name.toUpperCase()})',
          category: LogCategory.speech);
    } catch (e, stackTrace) {
      _logger.e('❌ [NEXA STT] Failed to start Nexa ASR processing',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  void _processSpeechResultWithNexa(SpeechResult result) {
    _logger.d(
        '🔄📥 [NEXA PROCESSING] Received speech result: "${result.text}" (final: ${result.isFinal}, confidence: ${result.confidence})',
        category: LogCategory.speech);
    ...
    try {
      ...
      if (result.text.isNotEmpty && result.text != defaultFallbackTranscript) {
        ...
        _logger.d(
            '📚 [NEXA PROCESSING] Added to recent texts (${_recentTexts.length} items)',
            category: LogCategory.speech);
      }
      ...
      _logger.d(
          '📤 [NEXA PROCESSING] Emitted raw speech result to speechResults stream',
          category: LogCategory.speech);

      // Try to enhance with Nexa LLM first (skip Gemma on Nexa devices — it won't be initialized)
      if (_nexaLlmService != null &&
          _nexaLlmService!.isReady &&
          _useEnhancement) {
        _logger.i(
            '✨ [NEXA PROCESSING] Nexa LLM available - attempting enhancement...',
            category: LogCategory.speech);
        _enhanceWithNexaLlm(result);
      } else if (_activeEngine != SpeechEngine.nexa_asr &&
          gemma3nService.isReady &&
          _useEnhancement) {
        _logger.i(
            '✨ [NEXA PROCESSING] Falling back to Gemma3n for enhancement...',
            category: LogCategory.speech);
        _enhanceWithGemma3n(result);
      } else {
        _logger.d(
            '📝 [NEXA PROCESSING] Using raw speech result (nexa llm ready: ${_nexaLlmService?.isReady}, gemma ready: ${gemma3nService.isReady}, enhancement enabled: $_useEnhancement)',
            category: LogCategory.speech);
        ...
        _logger.d(
            '📋➡️ [NEXA PROCESSING] Created and emitted basic caption: "${basicCaption.displayText}"',
            category: LogCategory.speech);
      }
    } catch (e, stackTrace) {
      _logger.e('❌ [NEXA PROCESSING] Error processing speech result',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
    }
  }
  ...
  void _enhanceWithNexaLlm(SpeechResult result) async {
    try {
      _logger.d('🚀 Starting Nexa LLM enhancement for: "${result.text}"',
          category: LogCategory.gemma);
      ...
      if (result.isFinal) {
        ...
        if (_gemmaProcessing) {
          _logger.w(
              '⚠️ Enhancement already processing another request, skipping: "${result.text}"',
              category: LogCategory.gemma);
          final basicCaption = EnhancedCaption.fromSpeechResult(result);
          _enhancedCaptionController.add(basicCaption);
          return;
        ...
        _logger.d('🔒 Enhancement mutex acquired for: "${result.text}"',
            category: LogCategory.gemma);
        ...
        try {
          ...
          if (currentFrame != null && _nexaLlmService!.supportsVision) {
            _logger.d(
                '🎥 Using visual context for Nexa LLM enhancement (${currentFrame.length} bytes)',
                category: LogCategory.gemma);
            ...
            _logger.d('📝 Using text-only Nexa LLM enhancement',
                category: LogCategory.gemma);
            enhancedText = await _nexaLlmService!.enhanceText(result.text);
          }

          _logger.d('✨ Nexa LLM enhancement result: "$enhancedText"',
              category: LogCategory.gemma);
          ...
          _logger.i(
              '📋 Created Nexa-enhanced caption: "${enhancedCaption.displayText}"',
              category: LogCategory.gemma);
        } finally {
          _gemmaProcessing = false;
          _logger.d('🔓 Enhancement mutex released',
              category: LogCategory.gemma);
        }
      } else {
      ...
      _gemmaProcessing = false;
      _logger.e('❌ Error enhancing with Nexa LLM',
          category: LogCategory.gemma, error: e, stackTrace: stackTrace);
  ...
  Future<void> _startStereoAudioCapture() async {
    try {
      _logger.i('🎧 [SPATIAL] Starting stereo audio recording...',
          category: LogCategory.speech);
      await _stereoAudioCapture.startRecording();
      _logger.i('✅ [SPATIAL] Stereo audio recording started',
          category: LogCategory.speech);
      ...
      _stereoAudioSubscription = _stereoAudioCapture.frames.listen((frame) {
        try {
          ...
          if (shouldLogFrame) {
            _logger.d(
                '🎧 [SPATIAL] Received stereo frame: L=${frame.left.length}, R=${frame.right.length} samples (frame #$_spatialFrameLogCounter)',
                category: LogCategory.speech);
          }
          ...
          _logger.w('⚠️ [SPATIAL] Could not update spatial caption service: $e',
              category: LogCategory.speech);
        }
      });

      _logger.i('🎧 [SPATIAL] Stereo audio processing setup complete',
          category: LogCategory.speech);
    } catch (e, stackTrace) {
      _logger.e('❌ [SPATIAL] Failed to start stereo audio capture',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
  ...
  Future<void> _stopStereoAudioCapture() async {
    try {
      _logger.i('🛑 [SPATIAL] Stopping stereo audio capture...',
          category: LogCategory.speech);
      ...
      _logger.i('✅ [SPATIAL] Stereo audio capture stopped',
          category: LogCategory.speech);
    } catch (e, stackTrace) {
      _logger.e('❌ [SPATIAL] Error stopping stereo audio capture',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
    }
  }
  ...
  void _enhanceWithGemma3n(SpeechResult result) async {
    try {
      ...
      if (result.isFinal) {
        ...
        _gemmaProcessing = true;
        _logger.d('🔒 Gemma mutex acquired for: "${result.text}"',
            category: LogCategory.gemma);
        ...
        try {
          ...
          if (currentFrame != null) {
            ...
            _logger.d('✅ enhanceTextWithVisualContext completed',
                category: LogCategory.gemma);
          } else {
            _logger.d('📝 Using text-only enhancement (no frame available)',
                category: LogCategory.gemma);
            _logger.d('🧠 Calling gemma3nService.enhanceText...',
                category: LogCategory.gemma);
            enhancedText = await gemma3nService.enhanceText(result.text);
            _logger.d('✅ enhanceText completed', category: LogCategory.gemma);
          }

          _logger.d('✨ Enhancement result: "$enhancedText"',
              category: LogCategory.gemma);
          ...
          _logger.d('▶️ STT was running continuously during Gemma inference',
              category: LogCategory.gemma);
        }
      } else {
        // For partial results, create a partial caption
      ...
      _gemmaProcessing = false;
      // _appleSpeechService.unblockAutoRestart(restartImmediately: true);
      _logger.e('❌ Error enhancing with Gemma 3n',
          category: LogCategory.gemma, error: e, stackTrace: stackTrace);
      _logger.d('▶️ STT was running continuously during Gemma error',
          category: LogCategory.gemma);
  ...
  void setEnhancementEnabled(bool enabled) {
    ...
    _logger.i('✨ Enhancement mode ${enabled ? 'enabled' : 'disabled'}',
        category: LogCategory.gemma);
  }
  ...
  Future<List<int>?> _getCurrentFrame() async {
    _logger.d('📸 Capturing frame via FrameCaptureService...',
        category: LogCategory.camera);
    try {
      ...
      final frameData = await _frameCaptureService.captureFrame().timeout(
        Duration(seconds: 5),
        onTimeout: () {
          _logger.w('⏱️ Frame capture timed out after 5 seconds',
              category: LogCategory.camera);
          return null;
        },
      );
      if (frameData != null) {
        _logger.d('✅ Frame captured: ${frameData.length} bytes',
            category: LogCategory.camera);
        return frameData;
      } else {
        _logger.w('⚠️ Frame capture returned null',
            category: LogCategory.camera);
        return null;
      }
    } catch (e, stackTrace) {
      _logger.e('❌ Failed to capture frame',
