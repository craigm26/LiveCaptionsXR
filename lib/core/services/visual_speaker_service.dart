...
class VisualSpeakerService {
  VisualSpeakerService({
    ...
    Duration captureInterval = const Duration(milliseconds: 500),
        ...
        _channel =
            methodChannel ?? const MethodChannel('live_captions_xr/visual_speaker_methods');
  ...
  Future<void> start() async {
    ...
    _frameTimer = Timer.periodic(_captureInterval, (_) {
      unawaited(_captureAndSendFrame());
    });
    // Prime the detector immediately.
    unawaited(_captureAndSendFrame());

    _isRunning = true;
    _logger.i(
      'VisualSpeakerService started with interval ${_captureInterval.inMilliseconds}ms',
      category: LogCategory.camera,
    );
  }
  ...
  Future<void> _captureAndSendFrame() async {
    if (_captureInFlight || !_isRunning) {
      return;
    }
    _captureInFlight = true;
    try {
      final rawBytes = await _cameraService.captureFrame();
      ...
      final frame = CameraFrame(
        ...
        height: 0,
        timestamp: timestamp,
        exposureDurationMicros: 0,
        iso: 0,
      );
      await _channel.invokeMethod('processCameraFrame', {
        'bytes': frame.bytes,
        'timestampMicros': frame.timestamp.microsecondsSinceEpoch,
        'format': 'jpeg',
        'width': frame.width,
        'height': frame.height,
      });
      ...
      _logger.e(
        'Unexpected error capturing camera frame',
        category: LogCategory.camera,
        error: e,
        stackTrace: stackTrace,
      );
  ...
  DetectedSpeaker? _mapDetectedSpeaker(Map<String, dynamic> data) {
    try {
      ...
      final timestampMicros =
          (data['timestampMicros'] as num?)?.toInt() ?? DateTime.now().microsecondsSinceEpoch;
      ...
      return DetectedSpeaker(
        ...
        boundingBox: rect,
        timestamp: DateTime.fromMicrosecondsSinceEpoch(timestampMicros, isUtc: true)
            .toLocal(),
        confidence: (data['confidence'] as num?)?.toDouble() ?? 0,
        state: _stateFromString(data['state'] as String?),
        landmarks: landmarks,
        worldTransform: transformRaw?.map((e) => (e as num).toDouble()).toList(),
      );
    } catch (e, stackTrace) {
      _logger.e(
