...
class WhisperService {
  ...
  static const Duration _emulatorTranscribeTimeout = Duration(seconds: 120);
  ...
  Future<bool> initialize({SpeechConfig? config}) async {
    ...
    try {
      ...
      if (_isEmulator && _config.whisperModel == 'base') {
        ...
        final tinySupported =
            _modelDownloadManager.availableModels.contains(tinyModelKey);
        if (tinySupported) {
          final tinyExists =
              await _modelDownloadManager.modelExists(tinyModelKey);
          final tinyComplete =
              await _modelDownloadManager.modelIsComplete(tinyModelKey);
          if (tinyExists && tinyComplete) {
            _config = _config.copyWith(whisperModel: 'tiny');
            _logger.w(
                '🧪 Emulator detected: using whisper-tiny for lower-latency inference',
                category: LogCategory.speech);
          } else {
            _logger.w(
                '🧪 Emulator detected: whisper-tiny not available, keeping whisper-base',
                category: LogCategory.speech);
          }
        } else {
          _logger.w(
              '🧪 Emulator detected: whisper-tiny model key is not configured; keeping whisper-base',
              category: LogCategory.speech);
        }
      }
      ...
      final modelComplete =
          await _modelDownloadManager.modelIsComplete(modelKey);
      ...
      if (resolvedModelPath == null) {
        _logger.i(
            '📦 No local Whisper file yet; materializing bundled/remote model to app storage...',
            category: LogCategory.speech);
        await _modelDownloadManager.downloadModel(modelKey);
      ...
      _logger.i('📁 Configured Whisper model path: $configuredModelPath',
          category: LogCategory.speech);

      if (resolvedModelPath == null) {
        _logger.e('❌ No readable Whisper model binary found in $modelDir',
            category: LogCategory.speech);
        throw Exception('Whisper model binary missing in $modelDir');
      }

      _resolvedModelPath = resolvedModelPath;
      _logger.i('📁 Resolved Whisper model binary: $_resolvedModelPath',
          category: LogCategory.speech);
      ...
      try {
        ...
        _logger.e('❌ Native Whisper GGML initialization failed: $nativeError',
            category: LogCategory.speech);
        _logger.w('⚠️ Whisper service will run in fallback mode',
            category: LogCategory.speech);
      ...
      _logger.e('❌ Failed to initialize Whisper service',
  ...
  Future<bool> startProcessing() async {
    ...
    try {
      ...
      _logger.e('❌ Failed to start Whisper processing',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
      _isProcessing = false;
  ...
  Future<SpeechResult> processAudioBuffer(Uint8List audioData) async {
    ...
    if (_whisper == null) {
      _logger.w('⚠️ Whisper GGML native library not available, using fallback',
          category: LogCategory.speech);
    ...
    try {
      ...
      final tempFile = File(
          '${tempDir.path}/whisper_audio_${DateTime.now().millisecondsSinceEpoch}.wav');
      ...
      final isActivityFallback = timedOutOnEmulator ||
      ...
      _logger.e('❌ Error processing audio with Whisper',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
  ...
  Future<SpeechResult> processWavFile(String wavFilePath,
    ...
    try {
      _logger.i('🧪 Debug WAV decode requested: $wavFilePath',
          category: LogCategory.speech);
      _sttEventController.add(const WhisperSTTEvent(
        progress: 0.2,
      ...
      final transcribeRequest = TranscribeRequest(
        ...
        isTranslate: _config.whisperTranslateToEnglish,
        isSpecialTokens:
            _isEmulator ? false : _config.whisperSuppressNonSpeechTokens,
        threads: _isEmulator ? 2 : 4,
        isVerbose: false,
        isNoTimestamps: true,
        noFallback: false,
      ...
      final normalizedResponseText = response.text.trim();
      final isActivityFallback = timedOut ||
          normalizedResponseText.isEmpty ||
          normalizedResponseText.toLowerCase() == 'listening...';
      ...
      _logger.i(
          '🧪 Debug WAV Whisper result: "${speechResult.text}" (confidence: ${speechResult.confidence})',
          category: LogCategory.speech);
      _speechResultController.add(speechResult);
      _sttEventController.add(const WhisperSTTEvent(
        progress: 1.0,
      ...
      _logger.e('❌ Error decoding debug WAV with Whisper',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
      final fallbackResult = SpeechResult(
        text: 'Listening...',
        confidence: 0.0,
  ...
  Future<void> stopProcessing() async {
    ...
    try {
      ...
      _logger.e('❌ Error stopping Whisper processing',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> dispose() async {
    try {
      ...
      _logger.e('❌ Error disposing Whisper service',
          category: LogCategory.speech, error: e, stackTrace: stackTrace);
    }
  }
}
