...
class ASRExample {
  ...
  Future<void> initializeASR() async {
    try {
      ...
      final loadResult = await _plugin.loadModel(
      ...
      if (loadResult['success'] == true) {
        ...
        print('⚠️ Gemma 3n model loading failed, ASR will use native-only mode');
  ...
  Future<String> transcribeAudio(
    ...
    try {
      print('🎙️ Transcribing audio (${audioBytes.length} bytes, ${isFinal ? 'final' : 'interim'})');
  ...
  Future<void> testErrorHandling() async {
    ...
    try {
      ...
      final result = await transcribeAudio(_generateSimulatedAudio(10, 'silence'));
      print('Result: "$result"');
    } catch (e) {
    ...
    switch (type) {
      ...
      case 'sentence':
        ...
        for (int i = 0; i < samples * 2; i++) {
          ...
          final envelope = Math.sin(Math.pi * time * 2) * 0.8 + 0.2; // Amplitude envelope
          final freq = 200 + 100 * Math.sin(2 * Math.pi * time * 3); // Varying frequency
          final value = (15000 * envelope * Math.sin(2 * Math.pi * freq * time)).round();
          buffer.add(value & 0xFF);
          buffer.add((value >> 8) & 0xFF);
        }
        break;
